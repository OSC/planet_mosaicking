...
def dtype_np2gdal(dtype_np):
    ...
    if promote_dtype is not None:
        warning = f"NumPy array data type ({dtype_np}) does not have equivalent GDAL data type and is not supported, but can be safely promoted to {promote_dtype(1).dtype}"
        warn(warning)
...
def interp2_fill_oob(X, Y, Zi, Xi, Yi, fillval=np.nan, coord_grace=True):
    ...
    if coord_grace:
        x_grace = (X[1] - X[0]) / 64
        y_grace = (Y[1] - Y[0]) / 16
        x_lfttest_val = x_lfttest_val - x_grace
        x_rgttest_val = x_rgttest_val + x_grace
        y_toptest_val = y_toptest_val - y_grace
        y_bottest_val = y_bottest_val + y_grace
...
def interp2_gdal(X, Y, Z, Xi, Yi, interp_str, extrapolate=False, oob_val=np.nan):
    ...
    Zi = ds_out.GetRasterBand(1).ReadAsArray().astype(np.float)
